...
class SmartphoneGUI:
    ...
    def create_widgets(self):
        ...
        #Photos app
        Label(self.main_frame, text="BnL Smartphone").grid(row=6, column=0, columnspan=3, sticky="w")
        ...
        #Mailbox App
        Label(self.main_frame, text="Number of Emails:").grid(row=10, column = 0, columnspan=3, sticky="w")
        Label(self.main_frame, textvariable=self.number_emails_var).grid(row=10, column=1, columnspan=1, sticky="w")

        Label(self.main_frame, text="Storage Used:").grid(row=11, column = 0, columnspan=3, sticky="w")
        Label(self.main_frame, textvariable=self.storage_used_var).grid(row=11, column=1, columnspan=1, sticky="w")

        Entry(self.main_frame, textvariable=self.sender_var, width=30).grid(row=12, column=0, columnspan=3, sticky="w")
        self.sender_var.set("Write the sender here")


        Entry(self.main_frame, textvariable=self.content_var, width=30).grid(row=12, column=2, columnspan=3, sticky="we")
        self.content_var.set("Write the content here")

        Button(self.main_frame, text="Receive Email", command=self.receive_email).grid(row=13, column=0, pady=5, sticky="w")
    ...
    def change_battery_mode(self):
        ...
        if battery_mode:
            ...
            self.battery_saver_var.set(value=f"Enabled")


        else:
            self.battery_saver_var.set(value=f"Disabled")
    ...
    def delete_photo(self):
        ...
        self.battery_used_var.set(f"{self.smartphone.battery}%")
        self.number_photos_var.set(str(self.smartphone.photos_app.num_photos()))
        self.storage_used_photos_var.set(
        f"{self.smartphone.photos_app.calculate_storage_used():.2f}GB")
